...
const LS_PREFS    = "kind_prefs_v2";   // bump de versión para incluir nuevos campos
...
const PREFS_DEFAULT = {
  ...
  temas:    [],
  portales: [],
  // NUEVO: intereses de usuario (cualquier cosa que se pueda agregar)
  intereses: [],
};

// ─── HELPERS localStorage ─────────────────────────────────────────────────────
function loadPrefs() {
  try { return { ...PREFS_DEFAULT, ...JSON.parse(localStorage.getItem(LS_PREFS)) }; }
...
function isOnboardingDone() { return localStorage.getItem(LS_ONBOARDING) === "true"; }
function markOnboardingDone() { localStorage.setItem(LS_ONBOARDING, "true"); }

// ─── FUENTES DISPONIBLES (expandidas) ────────────────────────────────────────
const ALL_PORTALS = [
  // Internacionales
  "BBC Mundo", "DW Español", "El País", "France 24",
  // Argentina
  "Infobae", "Clarín", "La Nación", "Página 12", "Télam", "Perfil",
  // México
  "El Universal", "Milenio", "El Financiero",
  // España
  "El Mundo", "La Vanguardia", "El Confidencial",
  // Otros LATAM
  "La Tercera", "El Tiempo", "El Comercio",
  // Temáticos
  "National Geographic", "MIT Technology Review",
];

// ─── TEMAS DISPONIBLES (expandidos) ──────────────────────────────────────────
const ALL_TEMAS = [
  { id: "Mundo",       label: "🌍 Mundo",       desc: "Geopolítica e internacional" },
  { id: "Ciencia",     label: "🔬 Ciencia",      desc: "Investigación y descubrimientos" },
  { id: "Ambiente",    label: "🌿 Ambiente",      desc: "Ecología y naturaleza" },
  { id: "Tecnología",  label: "💻 Tecnología",    desc: "IA, gadgets e innovación" },
  { id: "Salud",       label: "🏥 Salud",         desc: "Medicina y bienestar" },
  { id: "Economía",    label: "📈 Economía",      desc: "Mercados y finanzas" },
  { id: "Cultura",     label: "🎭 Cultura",       desc: "Arte, libros y cine" },
  { id: "Sociedad",    label: "🤝 Sociedad",      desc: "Comunidad y derechos" },
  { id: "Clima",       label: "🌦️ Clima",         desc: "Meteorología y fenómenos" },  // NUEVO
  { id: "Educación",   label: "📚 Educación",     desc: "Aprendizaje e innovación" },   // NUEVO
  { id: "Deportes",    label: "⚽ Deportes",      desc: "Fútbol y deportes regionales" }, // NUEVO
];

// ─── CATEGORÍAS PARA EL FEED ─────────────────────────────────────────────────
const CATEGORIES = [
  { id: "all",        label: "Todo" },
  ...
  { id: "Clima",      label: "🌦️ Clima" },  // NUEVO
];

const SECTIONS = [
  { id: "feed",      label: "Feed",             icon: "📰", desc: "Todas las noticias del día" },
  { id: "clima",     label: "Clima",             icon: "🌦️", desc: "Noticias del tiempo y ambiente" }, // NUEVO
...
const NEWS_FALLBACK = [
  ...
  { id: 3, portal: "BBC Mundo",  topic: "Ciencia",    title: "Células madre cardíacas: el ensayo que puede cambiar la medicina",   summary: "Un equipo internacional publicó resultados que podrían transformar el tratamiento del corazón.", tone: "positive", time: "1h", photo: "https://images.unsplash.com/photo-1579154204601-01588f351e67?w=600&h=360&fit=crop", read: 4, url: "https://www.bbc.com/mundo" },
  { id: 4, portal: "Infobae",    topic: "Economía",   title: "El desempleo juvenil en LATAM supera el 20% por segundo año",       summary: "El informe de la OIT revela que la recuperación post-pandemia no llegó al segmento más joven.", tone: "hard", time: "2h", photo: "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=600&h=360&fit=crop", read: 5, url: "https://www.infobae.com" },
  ...
  { id: 8, portal: "La Nación",  topic: "Cultura",    title: "80.000 personas y récord histórico en la Feria del Libro",         summary: "La edición 2026 superó todas las marcas anteriores. El 40% tenía menos de 30 años.", tone: "positive", time: "4h", photo: "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=600&h=360&fit=crop", read: 3, url: "https://www.lanacion.com.ar" },
];

const CLIMA_FALLBACK = [
  { id: "c1", portal: "SMN", topic: "Clima", title: "Alerta por tormentas fuertes en el centro del país", summary: "Se esperan lluvias intensas con granizo y vientos de hasta 90 km/h.", tone: "hard", time: "30 min", photo: "https://images.unsplash.com/photo-1504608524841-42584120d1ad?w=600&h=360&fit=crop", read: 2, url: "#", featured: true },
  { id: "c2", portal: "Meteored", topic: "Clima", title: "La ola de calor en el sur de Europa bate récords históricos", summary: "Las temperaturas superaron los 45°C en varias ciudades españolas esta semana.", tone: "hard", time: "1h", photo: "https://images.unsplash.com/photo-1601134467661-3d775b999c18?w=600&h=360&fit=crop", read: 3, url: "#" },
  { id: "c3", portal: "NASA", topic: "Clima", title: "El Niño debilita: se esperan lluvias más normales para el verano", summary: "Los modelos climáticos sugieren una transición hacia condiciones neutras hacia fin de año.", tone: "positive", time: "2h", photo: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=600&h=360&fit=crop", read: 4, url: "#" },
];
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* CLIMA */
  .clima-hero { background: linear-gradient(135deg, #0f2944 0%, #1a4a7a 60%, #2563a8 100%); border-radius: 14px; padding: 28px; margin-bottom: 20px; position: relative; overflow: hidden; }
  .clima-hero::before { content: ''; position: absolute; top: -40px; right: -40px; width: 200px; height: 200px; background: radial-gradient(circle, rgba(255,255,255,0.07) 0%, transparent 65%); border-radius: 50%; pointer-events: none; }
  .ch-label { font-size: 9.5px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: rgba(255,255,255,0.45); margin-bottom: 10px; display: flex; align-items: center; gap: 7px; }
  .ch-title { font-family: 'Lora', serif; font-size: 22px; font-weight: 700; color: white; line-height: 1.3; margin-bottom: 8px; }
  .ch-sub { font-size: 13px; font-weight: 300; color: rgba(255,255,255,0.6); line-height: 1.6; }
  .clima-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1px; background: var(--border); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
  .clima-card { background: var(--paper); padding: 16px; cursor: pointer; transition: background 0.18s; animation: fadeUp 0.4s ease forwards; opacity: 0; }
  .clima-card:hover { background: white; }
  .clima-card-img { width: 100%; height: 110px; border-radius: 7px; overflow: hidden; margin-bottom: 10px; }
  .clima-card-img img { width: 100%; height: 100%; object-fit: cover; display: block; }
  .clima-tag { font-size: 9px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--blue); margin-bottom: 6px; }
  .clima-card-title { font-family: 'Lora', serif; font-size: 13.5px; font-weight: 600; color: var(--ink); line-height: 1.4; margin-bottom: 8px; }
  .clima-card-meta { font-size: 10px; color: var(--muted); }
  ...
  /* ONBOARDING */
  .onboarding-banner { position: fixed; bottom: 0; left: 0; right: 0; z-index: 400; background: white; border-top: 1px solid var(--border); box-shadow: 0 -4px 24px rgba(0,0,0,0.08); padding: 16px 24px; display: flex; align-items: center; gap: 16px; animation: slideUp2 0.4s ease forwards; }
  ...
  .int-section-title { font-size: 11px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--muted); margin-bottom: 10px; margin-top: 20px; }
  .int-autosave { font-size: 11px; color: var(--green); font-weight: 500; display: flex; align-items: center; gap: 5px; margin-top: 8px; min-height: 18px; transition: opacity 0.3s; }

  /* FUENTES GRID (3 cols) */
  .portals-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 7px; margin-bottom: 4px; }
  .portal-pill { display: flex; align-items: center; justify-content: space-between; padding: 9px 12px; border: 1.5px solid var(--border); border-radius: 10px; cursor: pointer; background: white; transition: all 0.15s; }
  .portal-pill.selected { border-color: var(--green); background: var(--green-pale); }
  .portal-name { font-size: 12px; font-weight: 500; color: var(--ink-2); }
  .portal-check { width: 16px; height: 16px; border-radius: 50%; border: 1.5px solid var(--border); display: flex; align-items: center; justify-content: center; font-size: 9px; transition: all 0.15s; flex-shrink: 0; }
  .portal-pill.selected .portal-check { background: var(--green); border-color: var(--green); color: white; }

  /* TEMAS GRID */
  .topics-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; margin-bottom: 4px; }
  .topic-card { display: flex; align-items: center; gap: 10px; padding: 11px 14px; border: 1.5px solid var(--border); border-radius: 10px; cursor: pointer; background: white; transition: all 0.15s; }
  .topic-card.selected { border-color: var(--ink); background: var(--ink); }
  .topic-card-label { font-size: 13px; font-weight: 500; color: var(--ink-2); }
  .topic-card-desc { font-size: 11px; color: var(--muted); font-weight: 300; }
  .topic-card.selected .topic-card-label { color: white; }
  .topic-card.selected .topic-card-desc { color: rgba(255,255,255,0.5); }

  .region-select-full { width: 100%; padding: 11px 14px; border: 1.5px solid var(--border); border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 13px; color: var(--ink); background: white; outline: none; cursor: pointer; }
  .region-select-full:focus { border-color: var(--green); }

  /* HISTORIA */
  .history-card { background: white; border: 1px solid var(--border); border-radius: 14px; padding: 18px; margin-top: 20px; }
...
export default function Kind() {
  ...
  const [climaNoticias, setClimaNoticias] = useState(CLIMA_FALLBACK); // NUEVO
  ...
  // Preferencias — guardado automático en localStorage
  const [prefs,      setPrefs]      = useState(loadPrefs);
  const [prefsTemp,  setPrefsTemp]  = useState(loadPrefs);
  const [autoSaved,  setAutoSaved]  = useState(false); // NUEVO: indicador de guardado automático
    ...
    const cargarBackend = async () => {
      ...
      try {
        ...
        if (data.noticias && data.noticias.length > 0) {
          ...
          const sorted   = reordenarPorHistorial(noticias, loadHistory());
          setVisibleNews(sorted);
          setExtraPool([]);
          setBackendOk(true);
        }
        // NUEVO: clima desde el backend
        if (data.noticiasClima && data.noticiasClima.length > 0) {
          setClimaNoticias(data.noticiasClima);
        }
  ...
  // Cada vez que prefsTemp cambia, guarda en localStorage automáticamente
  useEffect(() => {
    savePrefs(prefsTemp);
    // Si la región cambió, actualizamos prefs activo también
    if (prefsTemp.region !== prefs.region) {
      setPrefs(prefsTemp);
    }
  ...
  function reordenarPorHistorial(noticias, historial) {
    ...
    const total = Object.values(historial).reduce((a, b) => a + b, 0);
    return [...noticias].sort((a, b) => {
      const pA = (historial[a.topic] || 0) / total;
      const pB = (historial[b.topic] || 0) / total;
      return pB - pA;
    });
  }
  ...
  // ── GUARDAR PREFERENCIAS (botón manual ya no es necesario pero lo dejamos) ─
  const handleDismissBanner = () => { markOnboardingDone(); setShowBanner(false); };
  ...
  const filtered = visibleNews.filter(n => {
    ...
    const portalOk = prefsTemp.portales.length === 0 || prefsTemp.portales.includes(n.portal);
      ...
      {showBanner && (
        <div className="onboarding-banner">
          ...
          <div className="ob-text">
            ...
            <div className="ob-sub">Elegí tu región, temas y fuentes. Se guarda automáticamente.</div>
          </div>
      ...
      <div className="page">
            ...
            {section === "clima" && (
              <div>
                <div className="clima-hero">
                  <div className="ch-label"><div className="live-dot" style={{ background:"rgba(255,255,255,0.6)" }} />Clima · {REGIONES_DISPONIBLES.find(r => r.id === prefs.region)?.nombre || "América Latina"}</div>
                  <div className="ch-title">🌦️ El tiempo hoy</div>
                  <div className="ch-sub">Fenómenos meteorológicos, alertas y cambio climático en tu región y el mundo.</div>
                </div>

                <div className="sec-head">
                  <div className="sec-title">Noticias de clima</div>
                  <span className="sec-count">{climaNoticias.length} noticias</span>
                </div>

                <div className="clima-grid">
                  {climaNoticias.map((n, i) => (
                    <div key={n.id || i} className="clima-card" style={{ animationDelay:`${i * 0.05}s` }} onClick={() => handleOpenNews(n.url, "Clima")}>
                      {n.photo && (
                        <div className="clima-card-img">
                          <img src={n.photo} alt={n.title} loading="lazy" onError={e => e.target.style.display="none"} />
                        </div>
                      )}
                      <div className="clima-tag">🌦️ CLIMA · {n.portal}</div>
                      <div className="clima-card-title">{n.title}</div>
                      <div className="clima-card-meta">{n.summary}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {section === "intereses" && (
              <div>
                <div className="intereses-intro">
                  ...
                  <div className="int-note">💡 Tus preferencias se guardan automáticamente en este navegador. No necesitás crear una cuenta.</div>
                </div>
                ...
                {/* Fuentes */}
                <div className="int-section-title">Fuentes preferidas <span style={{ fontWeight:300, textTransform:"none", letterSpacing:0 }}>— seleccioná las que querés priorizar</span></div>
                  ...
                  {ALL_PORTALS.map(p => (
                    <div
                      ...
                      onClick={() => setPrefsTemp(prev => ({
                        ...prev,
                        portales: prev.portales.includes(p)
                          ? prev.portales.filter(x => x !== p)
                          : [...prev.portales, p],
                      }))}
                    >
                ...
                {/* Temas */}
                <div className="int-section-title">Temas de interés <span style={{ fontWeight:300, textTransform:"none", letterSpacing:0 }}>— priorizamos estos en tu feed</span></div>
                <div className="topics-grid">
                  {ALL_TEMAS.map(t => (
                    <div
                      key={t.id}
                      className={`topic-card ${prefsTemp.temas.includes(t.id) ? "selected" : ""}`}
                      onClick={() => setPrefsTemp(prev => ({
                        ...prev,
                        temas: prev.temas.includes(t.id)
                          ? prev.temas.filter(x => x !== t.id)
                          : [...prev.temas, t.id],
                      }))}
                    >
                      <div>
                ...
                <div className="int-autosave" style={{ opacity: autoSaved ? 1 : 0 }}>
              ...
              <div>
                ...
                <div className="nos-cards">
                  {[
                    ...
                    { icon:"🎛️", title:"Feed personalizable",      text:"Elegís tu región, temas y fuentes. Kind los guarda automáticamente sin necesidad de cuenta." },
                    { icon:"🌦️", title:"Sección de clima",         text:"Noticias meteorológicas y fenómenos climáticos de tu región, actualizadas cada 2 horas." },
        ...
        <div className="mob-nav">
          <div className="mob-nav-inner">
            {[["feed","📰","Feed"],["clima","🌦️","Clima"],["brief","☀️","Brief"],["intereses","🎛️","Intereses"],["nosotros","✦","Kind"]].map(([id, icon, label]) => (
